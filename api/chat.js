export default async function handler(req, res) {
  ...
  try {
    ...
    const contents = safeHistory.map(item => ({ role: item.role === 'assistant' ? 'model' : 'user', parts: [{ text: String(item.text || '') }] }));
...
Mỗi câu trả lời nên có đủ thông tin cần thiết nhưng dễ đọc đối với người Việt học tiếng Trung.`;
    const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent', {
      ...
      body: JSON.stringify({ system_instruction: { parts: [{ text: systemInstruction }] }, contents, generationConfig: { temperature: 0.7, maxOutputTokens: 1000 } })
    });
